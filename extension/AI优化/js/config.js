				...
				const more = ui.create.div(
					...
					ui.joint`
						<div style="line-height: 24px; text-align: left">
							...
							<br />无名杀QQ频道：
							<br /><img style="width:243px" src="${lib.assetURL}extension/AI优化/img/promotion/qq.jpg">
...
commonConfigFold: {
     ...
     <div class="switch-header">
         <label class="config_switch">
             <input type="checkbox" ${lib.config.extension_AI优化_aiChangeGeneral ? 'checked' : ''}>
             <span class="config_slider"></span>
         </label>
         <span class="config_name">
         ...
         <span style="font-size:15px">
             游戏开始时可令一名AI更换武将牌，开启后生效
						...
						<div class="config_item switch-item" data-name="neiKey">
							...
							<div class="config_intro">
								<span style="font-size:15px">
									内奸可在出牌阶段亮明身份并加1点体力上限，建议与［身份局AI优化］搭配使用
	...
	loadPz: {
		...
		onclick() {
			...
			let initialCode = '//完整粘贴你保存的『AI优化』配置到等号右端\n_status.aiyh_config = ';
	...
	tip2: {
		clear: true,
		name: ui.joint`
			<div style="display: flex; justify-content: center">技能威胁度</div>
			<br /><span style="color: #FF3300">注意！</span>
				...
				saveInput: (code) => {
					try {
						...
						if (Object.prototype.toString.call(threaten) !== '[object Object]') {
							throw new Error('配置格式错误：类型不符');
						}
						game.saveExtensionConfig('AI优化', 'cf', threaten);
					} catch (e) {
						if (e.message?.includes('类型不符')) {
							alert(e.message);
						} else {
							alert('代码语法有错误，请仔细检查（' + e + '）');
						}
