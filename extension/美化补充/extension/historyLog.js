...
export function applyHistoryLogFeatures() {
if ((lib.config.show_log != "off") && lib.config.extension_美化补充_shoushalog) {
	lib.init.css(lib.assetURL + "extension/美化补充/css", "shoushalog");
}
if (lib.config.extension_美化补充_pause) {
	lib.init.css(lib.assetURL + "extension/美化补充/css", "pause");
	//历史记录栏美化
							...
							if (element.classList.contains("identity")) {
								if (element.classList.contains("guessing")) {
									if (get.mode() == "guozhan") {
										...
										var group = tar.group;
										if (group in group_dict) identity.setBackgroundImage(`extension/美化补充/image/pause/game_hist_gz_${group_dict[group]}.png`);
										else identity.setBackgroundImage(`extension/美化补充/image/pause/game_hist_gz_${group_dict["unknown"]}.png`);
										// 修改namebg的颜色
	...
	ui.create.pause = function () {
		...
		// node.style["background-image"] = ("url("" + lib.assetURL + "extension/美化补充/image/pause/43.png")");
...
// 精简历史记录
if (lib.config.extension_美化补充_conciselog) {
	game.log = function () {
