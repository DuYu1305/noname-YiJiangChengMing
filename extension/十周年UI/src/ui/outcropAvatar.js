...
export async function applyOutcropAvatar(characterName, node, outcropStyle) {
	...
	// 检查是否处于隐匿状态
	const isVice = node.classList.contains("avatar2") || node.classList.contains("deputy-avatar");
	...
	proto.setBackground = function (name, type, ...args) {
		...
		if (type === "character" && getOutcropStyle() !== "off") {
			const isPlayerAvatar =
				this.parentElement?.classList.contains("player") &&
				(this.classList.contains("avatar") ||
					this.classList.contains("avatar2") ||
					this.classList.contains("primary-avatar") ||
					this.classList.contains("deputy-avatar"));
			if (isPlayerAvatar) {
				const isVice = this.classList.contains("avatar2") || this.classList.contains("deputy-avatar");
				const characterName = (isVice ? this.parentElement?.name2 : this.parentElement?.name1 || this.parentElement?.name) || name;
				setTimeout(() => {
					if (characterName) {
						applyOutcropAvatar(characterName, this);
					}
				}, 0);
			}
		}
