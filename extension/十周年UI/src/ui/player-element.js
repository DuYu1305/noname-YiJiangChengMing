...
export function createPlayerElement(position, noclick) {
	...
	node.gainSkill.hiddenSkillIds = new Set([
		"feiyang",
		"bahu",
		"dcfeiyang",
		"mbfeiyang",
	...
	node.gainSkill.hiddenSkillNames = new Set(["飞扬", "跋扈"]);
	...
	node.gainSkill.gain = function (skill) {
	    if (this.hiddenSkillIds.has(skill) || this.hiddenSkillNames.has(get.translation(skill))) return;
