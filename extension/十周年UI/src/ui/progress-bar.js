...
const createAIProgressBar = isPhase => {
	...
	if (isShousha) {
		...
		container.style.cssText =
			"display:block;position:absolute;z-index:90;--w:122px;--h:calc(var(--w)*8/162);width:var(--w);height:var(--h);left:1.5px;bottom:-6px;";
		boxTime.data = 120;
...
const createTipImg = (className, imgName) => {
	...
	img.style.cssText = isShoushaSyle()
		...
		: "display:block;position:absolute;z-index:92;--w:129px;--h:calc(var(--w)*50/431);width:var(--w);height:var(--h);bottom:-10px;transform:scale(1.2);";
	return img;
};
