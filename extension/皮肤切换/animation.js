...
(function(newDuilib){
	...
	newDuilib.APNode = (function(){
		...
		APNode.prototype.update = function (e) {
			...
			if (domNode) {
				if (this.referFollow === true || !this.referBounds) {
		...
		AnimationPlayer.prototype.playSpine = function (sprite, position){
			...
			if (position != undefined) {
				for (let k in position) {
					if (k !== 'referNode' && k !== 'referFollow') {
						sprite[k] = position[k]
					}
				}
				if (position.referNode !== undefined) {
					sprite.referNode = position.referNode;
				} else if (position.parent !== undefined) {
					sprite.referNode = position.parent;
				}
				if (position.referFollow !== undefined) {
					sprite.referFollow = !!position.referFollow;
				} else if (position.follow !== undefined) {
